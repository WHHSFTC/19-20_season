...
class VisionFTCLib : OpenCvPipeline {
    ...
    private val means = ArrayList<Scalar>()
    ...
    private var tl: Telemetry?

    @Volatile
    var skystonePosition: SkystonePosition? = null
    var blocks: ArrayList<Rect>?
    ...
    @JvmOverloads
    constructor(firstSkystonePositionPercentage: Double, percentSpacing: Double, stoneWidth: Double, stoneHeight: Double, tl: Telemetry? = null) {
        defaultValues = false
     ...
     */
    fun drawRectangle(frame: Mat, rect: Rect?, color: Scalar?, thickness: Int): Mat {
    ...
    private fun setValues(width: Double, height: Double) {
        if (blocks == null) {
            ...
            blocks = ArrayList()
            blocks!!.add(Rect(Point(firstStonePosition - stoneWidth / 2, 0.50 * height - stoneHeight / 2),
                    Point(firstStonePosition + stoneWidth / 2, 0.50 * height + stoneHeight / 2)))
            blocks!!.add(Rect(Point(secondStonePosition - stoneWidth / 2, 0.50 * height - stoneHeight / 2),
                    Point(secondStonePosition + stoneWidth / 2, 0.50 * height + stoneHeight / 2)))
            blocks!!.add(Rect(Point(thirdStonePosition - stoneWidth / 2, 0.50 * height - stoneHeight / 2),
